"""
train_ls_jpeg.py - 训练JPEG+LS组合模型
"""
...
print(f"  JPEG质量范围: {JPEG_QUALITY_RANGE[0]} - {JPEG_QUALITY_RANGE[1]}")
print(f"  JPEG压缩概率: {JPEG_PROB}")
...
for epoch in range(EPOCHS):
    ...
    if acc > best_acc:
        best_acc = acc
        torch.save(model.state_dict(), os.path.join(MODEL_DIR, "xception_ls_jpeg.pth"))
...
df.to_csv(os.path.join(MODEL_DIR, "ls_jpeg_history.csv"), index=False)
print(f"训练记录已保存: {MODEL_DIR}/ls_jpeg_history.csv")